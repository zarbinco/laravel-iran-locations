...
class NeighborhoodBuilder extends LocationBuilder
{
    ...
    public function forArea(mixed $area): static
    {
        ...
        $this->whereHas('defaultArea', fn ($query) => $query->where('code', $value));
    ...
    public function filter(array $filters): static
    {
        ...
        $this->applyProvinceFilters($filters);
    ...
    private function applyProvinceFilters(array $filters): void
    {
        if (array_key_exists('province_id', $filters) && LocationFilterHelpers::filled($filters['province_id'])) {
            $provinceId = (int) $filters['province_id'];

            $this->whereHas('city', fn ($query) => $query->where('province_id', $provinceId));
        }

        if (($provinceCode = LocationFilterHelpers::string($filters['province_code'] ?? null)) !== null) {
            $this->whereHas('city.province', fn ($query) => $query->where('code', $provinceCode));
        }
    }
}
