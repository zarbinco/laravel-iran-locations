@extends('iran-locations::admin.layout', ['title' => 'Counties'])

@section('content')
    <div class="mb-4 flex items-center justify-between">
        <h2 class="text-lg font-semibold">Counties</h2>
        <a class="rounded bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-700" href="{{ route('iran-locations.admin.counties.create') }}">Create county</a>
    </div>
    ...
    <form method="GET" class="mb-4 grid gap-3 rounded border border-slate-200 bg-white p-4 sm:grid-cols-2 lg:grid-cols-6">
        ...
        <label class="text-sm">
            ...
            <select class="w-full rounded border border-slate-300 px-3 py-2" name="sort">
                @foreach (['name' => 'Name', '-name' => 'Name desc', 'province' => 'Province', '-province' => 'Province desc', 'code' => 'Code', '-code' => 'Code desc'] as $value => $label)
        ...
        <table class="min-w-full divide-y divide-slate-200 text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                @foreach ($counties as $county)
                    <tr>
                        <td class="px-4 py-3 font-medium">{{ $county->getAttribute('name_fa') }}</td>
                        <td class="px-4 py-3">{{ $county->getAttribute('code') }}</td>
                        <td class="px-4 py-3">{{ $county->province?->getAttribute('name_fa') ?? $county->getAttribute('province_id') }}</td>
                        <td class="px-4 py-3">{{ $county->getAttribute('source') }}</td>
                        <td class="px-4 py-3 text-right">
                            <a class="text-slate-700 underline" href="{{ route('iran-locations.admin.counties.edit', $county->getKey()) }}">Edit</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
    ...
    <div class="mt-4">{{ $counties->links() }}</div>
@endsection
